...
def test_create_trigger_shop_case(power_ops_client: PowerOpsClient):
    ...
    power_ops_client.powerops.upsert(shop_case)
    res_shop_case = power_ops_client.cogshop.retrieve_shop_case(shop_case.external_id)
    ...
    res = power_ops_client.powerops.shop_based_day_ahead_bid_process.shop_scenario.retrieve(scenario.external_id)
    assert isinstance(res, ShopScenario)
    res = power_ops_client.powerops.shop_based_day_ahead_bid_process.shop_model.retrieve(model.external_id)
    assert isinstance(res, ShopModel)
    res = power_ops_client.powerops.shop_based_day_ahead_bid_process.shop_file.retrieve(shop_files[0].external_id)
    assert isinstance(res, ShopFile)
    res = power_ops_client.powerops.shop_based_day_ahead_bid_process.shop_file.retrieve(shop_files[1].external_id)
    assert isinstance(res, ShopFile)
    ...
    power_ops_client.powerops.delete(external_id=shop_case, space="power_ops_instances")
    res_shop_case = power_ops_client.cogshop.retrieve_shop_case(shop_case.external_id)
