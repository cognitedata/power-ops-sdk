...
class AddWaterInTransitTestCase:
    ...
    discharge_values: list = field(default_factory=lambda: [0, 0, 0, 10, 10, 0])
    inflow_values: list = field(default_factory=lambda: [5] * 11)
    expected_shape: dict = field(default_factory=lambda: {0: 0, 180: 0.2, 240: 0.6, 300: 0.2})
    expected_values: list = field(default_factory=lambda: [7, 13, 13, 7] + [5] * 6)
    model: dict | None = None
    error: Exception | None = None


ADD_WATER_TEST_CASES = [
    AddWaterInTransitTestCase(case_id="default"),
    ...
    # AddWaterInTransitTestCase(case_id="basic", shape_type="shape_discharge", discharge_shape={"x": [0, 60, 120], "y": [0.1, 0.5, 0.4]}, expected_shape={0: 0.1, 60: 0.5, 120: 0.4}),
    # AddWaterInTransitTestCase(case_id="time_delay", shape_type="time_delay", discharge_shape=10, expected_shape={10: 1}),
...
def call_add_water_in_transit(cognite_client_mock, test_case):
    ...
    end_time_ms = inflow_times[-1].replace(tzinfo=timezone.utc).timestamp() * 1000
    ...
    expected_data = pd.Series(test_case.expected_values, index=inflow_times[:len(test_case.expected_values)])
...
def test_add_water_in_transit(cognite_client_mock, test_case):
    ...
    if test_case.error:
        with pytest.raises(test_case.error):
            call_add_water_in_transit(cognite_client_mock, test_case)
    else:
        call_add_water_in_transit(cognite_client_mock, test_case)
