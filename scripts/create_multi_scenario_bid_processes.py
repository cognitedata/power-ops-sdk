...
class Scenario:
            ...
            "transformations": [
                {
                    ...
                    "transformation": ''.join(['_' + c.lower() if c.isupper() else c for c in trans]).lstrip('_').upper()
                }
    ...
    def to_dict_v2(self) -> dict:
        ...
        return {
            "name": self.name,
            "time_series_external_id": self.time_series_external_id,
            "transformations": [{trans: {"parameters": {"relative_datapoints":[rdp.__dict__ for rdp in rpds]}} for trans, rpds in self.tranformations.items()}],
        }
...
def create_multi_scenario_demo_configs(num_scenarios_list: list[int], scenarios_file: Path, scenarios_file_v2: Path, bid_process_file: Path):
