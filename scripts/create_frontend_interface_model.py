...
def main():
    ...
    subprocess.check_call(["cdf-tk", "build", ".", "--clean", "--env=local"], env=env, cwd=ROOT / "interface_frontend")
    subprocess.check_call(["cdf-tk", "deploy", "--env=local"], env=env, cwd=ROOT / "interface_frontend")
