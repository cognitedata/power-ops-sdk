...
def main():
    ...
    # target_types = ["TIMESERIES", "timeSeries"]
    ...
    if not type_ids:
        ...
        return
    existing_timeseries = client.time_series.retrieve_multiple(external_ids=type_ids, ignore_unknown_ids=True)
    existing_timeseries_external_ids = {ts.external_id: ts for ts in existing_timeseries}
    relationships_to_delete = [r for r in relationships if r.target_external_id not in existing_timeseries_external_ids]
