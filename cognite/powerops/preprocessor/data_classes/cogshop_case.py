...
class CogShopCase:
    ...
    def retrieve_mapping_data(
    ...
    ) -> List[Tuple[TimeSeriesMapping, Union[float, pd.Series]]]:
        logger.debug(f"Retrieving time series data between {ms_to_datetime(start)} and {ms_to_datetime(end)}.")
        ...
        for mapping in mappings:
            ...
            datapoints.append(
                mapping,
                resample_and_aggregate(  # type: ignore[assignment]
                    datapoints=datapoints,
    ...
    def replace_time_series(
        self,
        mappings_data: List[Tuple[TimeSeriesMapping, Union[float, pd.Series]]],
    ) -> None:
        logger.debug("Replacing time series data.")
        for mapping, datapoints in mappings_data:
