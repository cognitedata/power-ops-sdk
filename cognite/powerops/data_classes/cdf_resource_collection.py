...
#from dm.schema import ModelTemplate


ExternalId = str

AddableResourceT = Union[
    "CogniteResource", list["CogniteResource"], "SequenceContent", list[ShopFileConfig]#, list[ModelTemplate]
]
...
class BootstrapResourceCollection(BaseModel):
    ...
    # dm resources:
    model_templates: dict[ExternalId, ModelTemplate] = {}
    ...
    def _add_resource(self, resource: CogniteResource | SequenceContent | ShopFileConfig): # | ModelTemplate):
        ...
        #elif isinstance(resource, ModelTemplate):
    ...
    def write_to_cdf(
        ...
        api_by_type = {
            ...
            **(
                ...
                if skip_dm
                    ...
                    "model_templates": client.dm.model_template,
                }
            ),
        }
        for resource_type, api in api_by_type.items():
            ...
            try:
                ...
                external_ids_to_update = [ids["externalId"] for ids in e.duplicated]
                print(f"The following {resource_type} already exists in CDF: {external_ids_to_update}")
                resources_to_update = [r for r in resources if r.external_id in external_ids_to_update]
                if hasattr(api, "update"):
                    ...
                    print("Deleting existing label definitions")
                    api.delete(external_id=external_ids_to_update)
                    print("Creating new label definitions")
                    api.create(resources_to_update)

        print(f"Processing {len(self.sequence_content)} sequences...")
        for sequence_external_id, sequence_data in self.sequence_content.items():
            if isinstance(sequence_data.data, pd.DataFrame):
                try:
                    ...
                    print(f"Failed to insert sequence dataframe for {sequence_external_id}: {e}")
            else:
                try:
                    ...
                    print(f"Failed to insert sequence rows for {sequence_external_id}: {e}")

        print(f"Processing {len(self.shop_file_configs)} files...")
