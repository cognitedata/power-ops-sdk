...
from cdf_resource_collection import BootstrapResourceCollection
from time_series_mapping import TimeSeriesMapping
...
def hafslund_mip_plant_time_series(
    reserve_group: str, shop_case_yaml: str, tco_paths: List[str]
) -> List[Tuple[str, Optional[str]]]:
    print("\033[92m LOADING HAFSLUND MIP FLAG \033[0m")
    shop_case = load_yaml(Path(shop_case_yaml), clean_data=True)
    tco_mapping = mapping_from_multiple_tco_files(paths=tco_paths, shop_model=shop_case["model"])
    plants = mip_plants(reserve_group=reserve_group, shop_case_yaml=shop_case_yaml)
    return _match_plants_and_mip_flag(plants=plants, mapping=tco_mapping)


def lyse_mip_plant_time_series(
    reserve_group: str, processed_yaml: str, mapping_yaml: str
) -> List[Tuple[str, Optional[str]]]:
    print("\033[92m LOADING LYSE MIP FLAG \033[0m")
    mapping = create_lyse_mapping(yaml_mapping_path=mapping_yaml, yaml_processed_path=processed_yaml)
    plants = mip_plants(reserve_group=reserve_group, shop_case_yaml=processed_yaml)
    return _match_plants_and_mip_flag(plants=plants, mapping=mapping)
