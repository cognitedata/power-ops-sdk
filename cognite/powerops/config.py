...
class WatercourseConfig(BaseModel):
    ...
    yaml_mapping_path: str = ""  # Lyse specific
    model_processed: str  # TODO: not used here
    model_mapping: Optional[str] = None  # Lyse specific
    tco_paths: Optional[List[str]] = None  # TODO: not used here - HEV specific
    rrs_ids: Optional[List[str]] = None  # HEV specific
    ...
    def load_plants_by_price_area(self, case: str) -> dict[str, list]:
        content = load_yaml(DATA / case / self.directory / self.model_raw, clean_data=True)
    ...
    def set_shop_yaml_paths(self, case):
        directory = "/".join((DATA / case / self.directory).relative_to(ROOT).parts)
        self.yaml_raw_path = f"{directory}/{self.model_raw}"  # TODO: create these as properties
        self.yaml_processed_path = f"{directory}/{self.model_processed}"
        self.yaml_mapping_path = f"{directory}/{self.model_mapping}"
...
class BidProcessConfig(BaseModel):
    ...
    def create_bid_matrix_generator_config_resources(
        ...
        for w in watercourses:
            sub_dict = w.load_plants_by_price_area(case)
