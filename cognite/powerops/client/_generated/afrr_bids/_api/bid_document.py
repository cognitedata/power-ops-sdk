...
from .bid_document_bids import BidDocumentBidsAPI
...
class BidDocumentAPI(NodeAPI[BidDocument, BidDocumentApply, BidDocumentList]):
    def __init__(self, client: CogniteClient, view_by_write_class: dict[type[DomainModelApply], dm.ViewId]):
        ...
        self.bids_edge = BidDocumentBidsAPI(client)
    ...
    def apply(
        ...
        between the nodes. For example, if any of `bids` or `alerts` are set, then these
        ...
        Examples:
            ...
            Create a new bid_document:
                ...
                >>> from cognite.powerops.client._generated.afrr_bids.data_classes import BidDocumentApply
                >>> client = AFRRBidsAPI()
    ...
    def list(
        ...
        Args:
            ...
            limit: Maximum number of bid documents to return. Defaults to 25. Set to -1, float("inf") or None to return all items.
            filter: (Advanced) If the filtering available in the above is not sufficient, you can write your own filtering which will be ANDed with the filter above.
            retrieve_edges: Whether to retrieve `bids` or `alerts` external ids for the bid documents. Defaults to True.
