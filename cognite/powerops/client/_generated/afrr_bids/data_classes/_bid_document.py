...
class BidDocument(DomainModel):
    ...
    Args:
        ...
        price_area: The PriceArea associated with the BidDocument (this should be shared).
...
class BidDocumentApply(DomainModelApply):
    ...
    def _to_instances_apply(
        ...
        write_view = (view_by_write_class and view_by_write_class.get(type(self))) or dm.ViewId(
            "power-ops-afrr-bids", "BidDocument", "1"
        )
        ...
        edge_type = dm.DirectRelationReference("power-ops-afrr-bids", "BidDocument.bids")
...
def _create_bid_document_filter(
    ...
    if price_area and isinstance(price_area, str):
        filters.append(
            dm.filters.Equals(
                view_id.as_property_ref("priceArea"), value={"space": "power-ops-afrr-bids", "externalId": price_area}
            )
        )
    ...
    if price_area and isinstance(price_area, list) and isinstance(price_area[0], str):
        filters.append(
            dm.filters.In(
                ...
                values=[{"space": "power-ops-afrr-bids", "externalId": item} for item in price_area],
            )
        )
