...
class BidRow(DomainModel):
    ...
    linked_bid: Union[BidRow, str, None] = Field(None, repr=False, alias="linkedBid")
...
class BidRowApply(DomainModelApply):
    ...
    is_block: Optional[bool] = Field(None, alias="isBlock")
    exclusive_group_id: Optional[str] = Field(None, alias="exclusiveGroupId")
    linked_bid: Union[BidRowApply, str, None] = Field(None, repr=False, alias="linkedBid")
    asset_type: Optional[str] = Field(None, alias="assetType")
    asset_id: Optional[str] = Field(None, alias="assetId")
    ...
    def _to_instances_apply(
        ...
        edge_type = dm.DirectRelationReference("power-ops-afrr-bids", "BidRow.alerts")
...
def _create_bid_row_filter(
    ...
    if linked_bid and isinstance(linked_bid, str):
        filters.append(
            dm.filters.Equals(
                view_id.as_property_ref("linkedBid"), value={"space": "power-ops-afrr-bids", "externalId": linked_bid}
            )
        )
    ...
    if linked_bid and isinstance(linked_bid, list) and isinstance(linked_bid[0], str):
        filters.append(
            dm.filters.In(
                ...
                values=[{"space": "power-ops-afrr-bids", "externalId": item} for item in linked_bid],
            )
        )
    ...
    if method and isinstance(method, str):
        filters.append(
            dm.filters.Equals(
                view_id.as_property_ref("method"), value={"space": "power-ops-afrr-bids", "externalId": method}
            )
        )
    ...
    if method and isinstance(method, list) and isinstance(method[0], str):
        filters.append(
            dm.filters.In(
                ...
                values=[{"space": "power-ops-afrr-bids", "externalId": item} for item in method],
            )
        )
