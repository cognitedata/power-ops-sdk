...
if TYPE_CHECKING:
    from cognite.powerops.client._generated.data_classes._benchmarking_production_obligation_day_ahead import (
    ...
    from cognite.powerops.client._generated.data_classes._power_asset import (
        PowerAsset,
        PowerAssetList,
        PowerAssetGraphQL,
        PowerAssetWrite,
...
class ShopModelWithAssets(DomainModel):
    ...
    power_assets: Optional[list[Union[PowerAsset, str, dm.NodeId]]] = Field(
        default=None, repr=False, alias="powerAssets"
    )
    production_obligations: Optional[list[Union[BenchmarkingProductionObligationDayAhead, str, dm.NodeId]]] = Field(
        default=None, repr=False, alias="productionObligations"
    )
...
class ShopModelWithAssetsWrite(DomainModelWrite):
    ...
    _outwards_edges: ClassVar[tuple[tuple[str, dm.DirectRelationReference], ...]] = (
        ("power_assets", dm.DirectRelationReference("power_ops_core", "ShopModelWithAssets")),
        ("production_obligations", dm.DirectRelationReference("power_ops_core", "ShopModelWithAssets")),
    )
    ...
    power_assets: Optional[list[Union[PowerAssetWrite, str, dm.NodeId]]] = Field(
        default=None, repr=False, alias="powerAssets"
    )
    production_obligations: Optional[list[Union[BenchmarkingProductionObligationDayAheadWrite, str, dm.NodeId]]] = (
...
class ShopModelWithAssetsList(DomainModelList[ShopModelWithAssets]):
    ...
    def shop_commands(self) -> ShopCommandsList:
        ...
        return ShopCommandsList(
            [item.shop_commands for item in self.data if isinstance(item.shop_commands, ShopCommands)]
        )
    ...
    def power_assets(self) -> PowerAssetList:
        ...
        return PowerAssetList(
            [item for items in self.data for item in items.power_assets or [] if isinstance(item, PowerAsset)]
        )
    ...
    def production_obligations(self) -> BenchmarkingProductionObligationDayAheadList:
        ...
        return BenchmarkingProductionObligationDayAheadList(
            [
                item
                for items in self.data
                for item in items.production_obligations or []
                if isinstance(item, BenchmarkingProductionObligationDayAhead)
            ]
        )
...
class ShopModelWithAssetsWriteList(DomainModelWriteList[ShopModelWithAssetsWrite]):
    ...
    def shop_model(self) -> ShopModelWriteList:
        ...
        return ShopModelWriteList(
            [item.shop_model for item in self.data if isinstance(item.shop_model, ShopModelWrite)]
        )
    ...
    def shop_commands(self) -> ShopCommandsWriteList:
        ...
        return ShopCommandsWriteList(
            [item.shop_commands for item in self.data if isinstance(item.shop_commands, ShopCommandsWrite)]
        )
    ...
    def power_assets(self) -> PowerAssetWriteList:
        ...
        return PowerAssetWriteList(
            [item for items in self.data for item in items.power_assets or [] if isinstance(item, PowerAssetWrite)]
        )
    ...
    def production_obligations(self) -> BenchmarkingProductionObligationDayAheadWriteList:
        ...
        return BenchmarkingProductionObligationDayAheadWriteList(
            [
                item
                for items in self.data
                for item in items.production_obligations or []
                if isinstance(item, BenchmarkingProductionObligationDayAheadWrite)
            ]
        )
...
def _create_shop_model_with_asset_filter(
    ...
    if (
        shop_model
        and isinstance(shop_model, Sequence)
        and not isinstance(shop_model, str)
        and not is_tuple_id(shop_model)
    ):
        filters.append(
            dm.filters.In(
                view_id.as_property_ref("shopModel"), values=[as_instance_dict_id(item) for item in shop_model]
            )
        )
    if isinstance(shop_commands, str | dm.NodeId | dm.DirectRelationReference) or is_tuple_id(shop_commands):
        filters.append(
            dm.filters.Equals(view_id.as_property_ref("shopCommands"), value=as_instance_dict_id(shop_commands))
        )
    if (
        shop_commands
        and isinstance(shop_commands, Sequence)
        and not isinstance(shop_commands, str)
        and not is_tuple_id(shop_commands)
    ):
        filters.append(
            dm.filters.In(
                view_id.as_property_ref("shopCommands"), values=[as_instance_dict_id(item) for item in shop_commands]
            )
        )
...
class _ShopModelWithAssetsQuery(NodeQueryCore[T_DomainModelList, ShopModelWithAssetsList]):
    ...
    def __init__(
        ...
        if (
            _BenchmarkingProductionObligationDayAheadQuery not in created_types
