...
class CustomBidMatrixAlertsAPI(EdgeAPI):
    def list(
        self,
        from_custom_bid_matrix: str | list[str] | dm.NodeId | list[dm.NodeId] | None = None,
        from_custom_bid_matrix_space: str = DEFAULT_INSTANCE_SPACE,
        ...
        """List alert edges of a custom bid matrix.

        Args:
            from_custom_bid_matrix: ID of the source custom bid matrix.
            from_custom_bid_matrix_space: Location of the custom bid matrixes.
        ...
        Examples:
            ...
            List 5 alert edges connected to "my_custom_bid_matrix":

                >>> from cognite.powerops.client._generated.day_ahead_bid import DayAheadBidAPI
                >>> client = DayAheadBidAPI()
                >>> custom_bid_matrix = client.custom_bid_matrix.alerts_edge.list("my_custom_bid_matrix", limit=5)

        """
        filter_ = _create_edge_filter(
            dm.DirectRelationReference("power-ops-types", "calculationIssue"),
            from_custom_bid_matrix,
            from_custom_bid_matrix_space,
            to_alert,
            to_alert_space,
            external_id_prefix,
            space,
