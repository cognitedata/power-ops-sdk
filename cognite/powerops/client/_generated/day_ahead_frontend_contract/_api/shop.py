...
from cognite.powerops.client._generated.day_ahead_frontend_contract.data_classes._shop import _SHOP_PROPERTIES_BY_FIELD
...
def _retrieve_timeseries_external_ids_with_extra_price_scenarios(
    ...
    if isinstance(extra_properties, str):
        ...
        extra_list = extra_properties
    has_data = dm.filters.HasData([dm.ContainerId("dayAheadFrontendContractModel", "SHOP")], [view_id])
...
class SHOPAPI(TypeAPI[SHOP, SHOPApply, SHOPList]):
    ...
    def apply(self, shop: SHOPApply | Sequence[SHOPApply], replace: bool = False) -> dm.InstancesApplyResult:
        ...
        """
        if isinstance(shop, SHOPApply):
            instances = shop.to_instances_apply(self._view_by_write_class)
        else:
            instances = SHOPApplyList(shop).to_instances_apply(self._view_by_write_class)
    ...
    @overload
    def retrieve(self, external_id: str) -> SHOP:
        ...

    @overload
    def retrieve(self, external_id: Sequence[str]) -> SHOPList:
        ...
