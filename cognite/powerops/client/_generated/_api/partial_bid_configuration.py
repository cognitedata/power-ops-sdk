...
class PartialBidConfigurationAPI(NodeAPI[PartialBidConfiguration, PartialBidConfigurationWrite, PartialBidConfigurationList, PartialBidConfigurationWriteList]):
    ...
    ) -> Iterator[PartialBidConfigurationList]:
        """Iterate over partial bid configurations

        Args:
            chunk_size: The number of partial bid configurations to return in each iteration. Defaults to 100.
            name: The name to filter on.
            name_prefix: The prefix of the name to filter on.
            method: The method to filter on.
            method_prefix: The prefix of the method to filter on.
            ...
            'skip'.'skip' will not retrieve any connections, 'identifier' will only retrieve the identifier of the
            connected items, and 'full' will retrieve the full connected items.
            limit: Maximum number of partial bid configurations to return. Defaults to None, which will return all items.
            cursors: (Advanced) Cursor to use for pagination. This can be used to resume an iteration from a
                specific point. See example below for more details.

        Returns:
            Iteration of partial bid configurations

        Examples:

            Iterate partial bid configurations in chunks of 100 up to 2000 items:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for partial_bid_configurations in client.partial_bid_configuration.iterate(chunk_size=100, limit=2000):
                ...     for partial_bid_configuration in partial_bid_configurations:
                ...         print(partial_bid_configuration.external_id)

            Iterate partial bid configurations in chunks of 100 sorted by external_id in descending order:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for partial_bid_configurations in client.partial_bid_configuration.iterate(
                ...     chunk_size=100,
                ...     sort_by="external_id",
            ...
            Iterate partial bid configurations in chunks of 100 and use cursors to resume the iteration:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for first_iteration in client.partial_bid_configuration.iterate(chunk_size=100, limit=2000):
