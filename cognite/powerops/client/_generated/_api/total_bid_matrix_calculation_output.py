...
from cognite.powerops.client._generated.data_classes._total_bid_matrix_calculation_output import (
    TotalBidMatrixCalculationOutputQuery,
...
class TotalBidMatrixCalculationOutputAPI(NodeAPI[TotalBidMatrixCalculationOutput, TotalBidMatrixCalculationOutputWrite, TotalBidMatrixCalculationOutputList, TotalBidMatrixCalculationOutputWriteList]):
    ...
    def retrieve(
        ...
        Args:
            ...
            retrieve_connections: Whether to retrieve `function_input`, `alerts` and `bid_document` for the total bid
            matrix calculation outputs. Defaults to 'skip'.'skip' will not retrieve any connections, 'identifier' will
        ...
        Examples:
            ...
            Retrieve total_bid_matrix_calculation_output by id:
                ...
                >>> total_bid_matrix_calculation_output = client.total_bid_matrix_calculation_output.retrieve(
                ...     "my_total_bid_matrix_calculation_output"
                ... )
    ...
    def search(
        ...
        Args:
            ...
            limit: Maximum number of total bid matrix calculation outputs to return. Defaults to 25.
        ...
        Examples:
           ...
           Search for 'my_total_bid_matrix_calculation_output' in all text properties:
                ...
                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> total_bid_matrix_calculation_outputs = client.total_bid_matrix_calculation_output.search(
                ...     'my_total_bid_matrix_calculation_output'
                ... )
    ...
    def histogram(
        ...
        Args:
            ...
            external_id_prefix: The prefix of the external ID to filter on.
            space: The space to filter on.
            limit: Maximum number of total bid matrix calculation outputs to return.
    ...
    def select(self) -> TotalBidMatrixCalculationOutputQuery:
        """Start selecting from total bid matrix calculation outputs."""
        return TotalBidMatrixCalculationOutputQuery(self._client)
        ...
        if retrieve_connections == "full":
            builder.extend(
                factory.from_direct_relation(
                    TotalBidMatrixCalculationInput._view_id,
            ...
            builder.extend(
                factory.from_direct_relation(
                    BidDocumentDayAhead._view_id,
                    ViewPropertyId(self._view_id, "bidDocument"),
                    has_container_fields=True,
                )
            )
    ...
    ) -> Iterator[TotalBidMatrixCalculationOutputList]:
        """Iterate over total bid matrix calculation outputs

        Args:
            chunk_size: The number of total bid matrix calculation outputs to return in each iteration. Defaults to 100.
            workflow_execution_id: The workflow execution id to filter on.
            workflow_execution_id_prefix: The prefix of the workflow execution id to filter on.
            min_workflow_step: The minimum value of the workflow step to filter on.
            max_workflow_step: The maximum value of the workflow step to filter on.
            ...
            matrix calculation outputs. Defaults to 'skip'.'skip' will not retrieve any connections, 'identifier' will
            only retrieve the identifier of the connected items, and 'full' will retrieve the full connected items.
            limit: Maximum number of total bid matrix calculation outputs to return. Defaults to None, which will return all items.
            cursors: (Advanced) Cursor to use for pagination. This can be used to resume an iteration from a
                specific point. See example below for more details.

        Returns:
            Iteration of total bid matrix calculation outputs

        Examples:

            Iterate total bid matrix calculation outputs in chunks of 100 up to 2000 items:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for total_bid_matrix_calculation_outputs in client.total_bid_matrix_calculation_output.iterate(chunk_size=100, limit=2000):
                ...     for total_bid_matrix_calculation_output in total_bid_matrix_calculation_outputs:
                ...         print(total_bid_matrix_calculation_output.external_id)

            Iterate total bid matrix calculation outputs in chunks of 100 sorted by external_id in descending order:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for total_bid_matrix_calculation_outputs in client.total_bid_matrix_calculation_output.iterate(
                ...     chunk_size=100,
                ...     sort_by="external_id",
            ...
            Iterate total bid matrix calculation outputs in chunks of 100 and use cursors to resume the iteration:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for first_iteration in client.total_bid_matrix_calculation_output.iterate(chunk_size=100, limit=2000):
