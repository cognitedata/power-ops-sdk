...
class PartialBidMatrixCalculationInputAPI(
    NodeAPI[
        PartialBidMatrixCalculationInput,
        PartialBidMatrixCalculationInputWrite,
        PartialBidMatrixCalculationInputList,
    ...
    def retrieve(
        ...
        as_child_class: (
            SequenceNotStr[
                Literal[
                    "MultiScenarioPartialBidMatrixCalculationInput", "WaterValueBasedPartialBidMatrixCalculationInput"
                ]
            ]
            | None
        ) = None,
    ...
    def search(
        ...
        properties: (
            PartialBidMatrixCalculationInputTextFields
            | SequenceNotStr[PartialBidMatrixCalculationInputTextFields]
            | None
        ) = None,
        ...
        sort_by: (
            PartialBidMatrixCalculationInputFields | SequenceNotStr[PartialBidMatrixCalculationInputFields] | None
        ) = None,
    ...
    ) -> Iterator[PartialBidMatrixCalculationInputList]:
        """Iterate over partial bid matrix calculation inputs

        Args:
            chunk_size: The number of partial bid matrix calculation inputs to return in each iteration. Defaults to 100.
            workflow_execution_id: The workflow execution id to filter on.
            workflow_execution_id_prefix: The prefix of the workflow execution id to filter on.
            min_workflow_step: The minimum value of the workflow step to filter on.
            max_workflow_step: The maximum value of the workflow step to filter on.
            ...
            'identifier' will only retrieve the identifier of the connected items, and 'full' will retrieve the full
            connected items.
            limit: Maximum number of partial bid matrix calculation inputs to return. Defaults to None, which will return all items.
            cursors: (Advanced) Cursor to use for pagination. This can be used to resume an iteration from a
                specific point. See example below for more details.

        Returns:
            Iteration of partial bid matrix calculation inputs

        Examples:

            Iterate partial bid matrix calculation inputs in chunks of 100 up to 2000 items:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for partial_bid_matrix_calculation_inputs in client.partial_bid_matrix_calculation_input.iterate(chunk_size=100, limit=2000):
                ...     for partial_bid_matrix_calculation_input in partial_bid_matrix_calculation_inputs:
                ...         print(partial_bid_matrix_calculation_input.external_id)

            Iterate partial bid matrix calculation inputs in chunks of 100 sorted by external_id in descending order:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for partial_bid_matrix_calculation_inputs in client.partial_bid_matrix_calculation_input.iterate(
                ...     chunk_size=100,
                ...     sort_by="external_id",
            ...
            Iterate partial bid matrix calculation inputs in chunks of 100 and use cursors to resume the iteration:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for first_iteration in client.partial_bid_matrix_calculation_input.iterate(chunk_size=100, limit=2000):
