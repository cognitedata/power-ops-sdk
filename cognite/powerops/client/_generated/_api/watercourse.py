...
from cognite.powerops.client._generated.data_classes._watercourse import (
    WatercourseQuery,
...
class WatercourseAPI(NodeAPI[Watercourse, WatercourseWrite, WatercourseList, WatercourseWriteList]):
    ...
    ) -> Iterator[WatercourseList]:
        """Iterate over watercourses

        Args:
            chunk_size: The number of watercourses to return in each iteration. Defaults to 100.
            name: The name to filter on.
            name_prefix: The prefix of the name to filter on.
            display_name: The display name to filter on.
            display_name_prefix: The prefix of the display name to filter on.
            ...
            filter: (Advanced) If the filtering available in the above is not sufficient,
                you can write your own filtering which will be ANDed with the filter above.
            limit: Maximum number of watercourses to return. Defaults to None, which will return all items.
            cursors: (Advanced) Cursor to use for pagination. This can be used to resume an iteration from a
                specific point. See example below for more details.

        Returns:
            Iteration of watercourses

        Examples:

            Iterate watercourses in chunks of 100 up to 2000 items:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for watercourses in client.watercourse.iterate(chunk_size=100, limit=2000):
                ...     for watercourse in watercourses:
                ...         print(watercourse.external_id)

            Iterate watercourses in chunks of 100 sorted by external_id in descending order:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for watercourses in client.watercourse.iterate(
                ...     chunk_size=100,
                ...     sort_by="external_id",
            ...
            Iterate watercourses in chunks of 100 and use cursors to resume the iteration:

                >>> from cognite.powerops.client._generated import PowerOpsModelsClient
                >>> client = PowerOpsModelsClient()
                >>> for first_iteration in client.watercourse.iterate(chunk_size=100, limit=2000):
