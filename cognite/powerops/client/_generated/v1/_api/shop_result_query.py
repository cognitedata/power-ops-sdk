...
class ShopResultQueryAPI(QueryAPI[T_DomainModel, T_DomainModelList]):
    ...
    def alerts(
        ...
        Args:
            ...
            retrieve_case: Whether to retrieve the case
                for each shop result or not.
    ...
    def output_time_series(
        ...
        limit: int = DEFAULT_QUERY_LIMIT,
        retrieve_case: bool = False,
    ) -> ShopTimeSeriesQueryAPI[T_DomainModel, T_DomainModelList]:
        ...
        Args:
            ...
            filter: (Advanced) Filter applied to node. If the filtering available in the
                above is not sufficient, you can write your own filtering which will be ANDed with the filter above.
            limit: Maximum number of output time series edges to return.
                Defaults to 3. Set to -1, float("inf") or None to return all items.
            retrieve_case: Whether to retrieve the case
                for each shop result or not.
        ...
        has_data = dm.filters.HasData(views=[view_id])
        node_filter = _create_shop_time_series_filter(
            view_id,
            object_type,
            object_type_prefix,
            object_name,
