...
if TYPE_CHECKING:
    ...
    from ._bid_method_custom import BidMethodCustom, BidMethodCustomGraphQL, BidMethodCustomWrite
...
CustomBidMatrixFields = Literal["resource_cost", "matrix", "asset_type", "asset_id", "is_processed"]
...
class CustomBidMatrixGraphQL(GraphQLCore):
    """This represents the reading version of custom bid matrix, used
    ...
    Args:
        ...
        data_record: The data record of the custom bid matrix node.
    ...
    view_id = dm.ViewId("sp_powerops_models", "CustomBidMatrix", "1")
    ...
    method: Optional[BidMethodCustomGraphQL] = Field(None, repr=False)
    ...
    @field_validator("alerts", "method", mode="before")
    ...
    def as_read(self) -> CustomBidMatrix:
        """Convert this GraphQL format of custom bid matrix to the reading format."""
        if self.data_record is None:
            raise ValueError("This object cannot be converted to a read format because it lacks a data record.")
        return CustomBidMatrix(
            space=self.space,
    ...
    def as_write(self) -> CustomBidMatrixWrite:
        """Convert this GraphQL format of custom bid matrix to the writing format."""
        return CustomBidMatrixWrite(
            space=self.space,
            external_id=self.external_id,
...
class CustomBidMatrixWrite(BidMatrixWrite):
    ...
    def _to_instances_write(
        ...
        write_view = (view_by_read_class or {}).get(
            CustomBidMatrix, dm.ViewId("sp_powerops_models", "CustomBidMatrix", "1")
        )
...
class CustomBidMatrixApply(CustomBidMatrixWrite):
    def __new__(cls, *args, **kwargs) -> CustomBidMatrixApply:
        warnings.warn(
            "CustomBidMatrixApply is deprecated and will be removed in v1.0. Use CustomBidMatrixWrite instead."
...
class CustomBidMatrixList(DomainModelList[CustomBidMatrix]):
    ...
    def as_write(self) -> CustomBidMatrixWriteList:
        """Convert these read versions of custom bid matrix to the writing versions."""
        return CustomBidMatrixWriteList([node.as_write() for node in self.data])

    def as_apply(self) -> CustomBidMatrixWriteList:
...
class CustomBidMatrixWriteList(DomainModelWriteList[CustomBidMatrixWrite]):
    """List of custom bid matrixes in the writing version."""

    _INSTANCE = CustomBidMatrixWrite


class CustomBidMatrixApplyList(CustomBidMatrixWriteList): ...
