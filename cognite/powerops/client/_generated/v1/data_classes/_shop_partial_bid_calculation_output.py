...
class ShopPartialBidCalculationOutputGraphQL(GraphQLCore):
    """This represents the reading version of shop partial bid calculation output, used
    ...
    view_id = dm.ViewId("sp_powerops_models", "ShopPartialBidCalculationOutput", "1")
    ...
    bid_matrix_raw: Optional[MultiScenarioMatrixRawGraphQL] = Field(None, repr=False, alias="bidMatrixRaw")
    input_: Optional[ShopPartialBidCalculationInputGraphQL] = Field(None, repr=False, alias="input")
    ...
    @field_validator("alerts", "bid_matrix_raw", "input_", mode="before")
    ...
    def as_read(self) -> ShopPartialBidCalculationOutput:
        """Convert this GraphQL format of shop partial bid calculation output to the reading format."""
        if self.data_record is None:
            raise ValueError("This object cannot be converted to a read format because it lacks a data record.")
        return ShopPartialBidCalculationOutput(
            space=self.space,
            ...
            bid_matrix_raw=(
                self.bid_matrix_raw.as_read() if isinstance(self.bid_matrix_raw, GraphQLCore) else self.bid_matrix_raw
            ),
    ...
    def as_write(self) -> ShopPartialBidCalculationOutputWrite:
        """Convert this GraphQL format of shop partial bid calculation output to the writing format."""
