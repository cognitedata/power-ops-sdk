...
from cognite.powerops.client.dm_client.data_classes import Mapping, MappingApply, MappingList
...
class MappingsAPI(TypeAPI[Mapping, MappingApply, MappingList]):
    ...
    def delete(self, external_id: str | Sequence[str]) -> dm.InstancesDeleteResult:
        if isinstance(external_id, str):
            ...
            return self._client.data_modeling.instances.delete(nodes=[(MappingApply.space, id) for id in external_id])
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> Mapping | MappingList:
        if isinstance(external_id, str):
            mapping = self._retrieve(("cogShop", external_id))
            ...
            mappings = self._retrieve([("cogShop", ext_id) for ext_id in external_id])
            transformation_edges = self.transformations.retrieve(external_id)
            self._set_transformations(mappings, transformation_edges)

            return mappings

    def list(self, limit: int = INSTANCES_LIST_LIMIT_DEFAULT) -> MappingList:
