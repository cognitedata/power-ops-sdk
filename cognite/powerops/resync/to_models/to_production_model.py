...
def _extract_watercourse_data(
    ...
    for plant_name, attributes in shop_case["model"]["plant"].items():
        ...
        if mapping:
            mappings = dict(
                water_value_time_series=TimeSeries(external_id=mapping.water_value) if mapping.water_value else None,
                ...
                feeding_fee_time_series=TimeSeries(external_id=mapping.feeding_fee) if mapping.feeding_fee else None,
                ...
                head_direct_time_series=TimeSeries(external_id=mapping.head_direct) if mapping.head_direct else None,
            )
        else:
            mappings = {}
