...
from cognite.powerops.client.dm_client.data_classes import FileRef, FileRefApply, FileRefList
...
class FileRefsAPI(TypeAPI[FileRef, FileRefApply, FileRefList]):
    ...
    def delete(self, external_id: str | Sequence[str]) -> dm.InstancesDeleteResult:
        if isinstance(external_id, str):
            ...
            return self._client.data_modeling.instances.delete(nodes=[(FileRefApply.space, id) for id in external_id])
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> FileRef | FileRefList:
        if isinstance(external_id, str):
            return self._retrieve(("cogShop", external_id))
        else:
            return self._retrieve([("cogShop", ext_id) for ext_id in external_id])
