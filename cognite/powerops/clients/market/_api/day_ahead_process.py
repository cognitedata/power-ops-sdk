...
from cognite.powerops.clients.market.data_classes import DayAheadProces, DayAheadProcesApply, DayAheadProcesList
...
class DayAheadProcesIncrementalMappingsAPI:
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> dm.EdgeList:
        ...
        is_edge_type = f.Equals(
            ["edge", "type"],
            {"space": "power-ops", "externalId": "DayAheadProcess.incrementalMapping"},
        )
        if isinstance(external_id, str):
            is_day_ahead_proces = f.Equals(
                ["edge", "startNode"],
...
class DayAheadProcessAPI(TypeAPI[DayAheadProces, DayAheadProcesApply, DayAheadProcesList]):
    def __init__(self, client: CogniteClient):
        super().__init__(
            client=client,
            sources=dm.ViewId("power-ops", "DayAheadProcess", "2812da73854f7a"),
    ...
    def _set_incremental_mapping(
        ...
        for day_ahead_proces in day_ahead_process:
            node_id = day_ahead_proces.id_tuple()
            if node_id in edges_by_start_node:
                day_ahead_proces.incremental_mapping = [
                    edge.end_node.external_id for edge in edges_by_start_node[node_id]
                ]
