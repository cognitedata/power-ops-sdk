...
from cognite.powerops.clients.market.data_classes import RKOMProces, RKOMProcesApply, RKOMProcesList
...
class RKOMProcesIncrementalMappingsAPI:
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> dm.EdgeList:
        ...
        is_edge_type = f.Equals(
            ["edge", "type"],
            {"space": "power-ops", "externalId": "RKOMProcess.incrementalMapping"},
        )
...
class RKOMProcessAPI(TypeAPI[RKOMProces, RKOMProcesApply, RKOMProcesList]):
    def __init__(self, client: CogniteClient):
        super().__init__(
            client=client,
            sources=dm.ViewId("power-ops", "RKOMProcess", "e41dfc7236eb4b"),
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> RKOMProces | RKOMProcesList:
        if isinstance(external_id, str):
            ...
            rkom_proces.incremental_mapping = [edge.end_node.external_id for edge in incremental_mapping_edges]
            ...
            incremental_mapping_edges = self.incremental_mappings.retrieve(external_id)
            self._set_incremental_mapping(rkom_process, incremental_mapping_edges)
    ...
    @staticmethod
    def _set_incremental_mapping(rkom_process: Sequence[RKOMProces], incremental_mapping_edges: Sequence[dm.Edge]):
        ...
        for rkom_proces in rkom_process:
            ...
            if node_id in edges_by_start_node:
                rkom_proces.incremental_mapping = [edge.end_node.external_id for edge in edges_by_start_node[node_id]]
