...
def _get_latest_external_id(client: PowerOpsClient, data_set_type: DataSetType) -> str:
    ...
    if data_set_type == "READ":
        external_id = getattr(dataset_config, "read_data_set", None)
    elif data_set_type == "WRITE":
        external_id = getattr(dataset_config, "write_data_set", None)
    elif data_set_type == "MONITOR":
        external_id = getattr(dataset_config, "monitor_data_set", None)
    elif data_set_type == "PROCESS":
        external_id = getattr(dataset_config, "process_data_set", None)
    else:
...
def get_dataset_id(client: PowerOpsClient, data_set_type: DataSetType = "READ") -> int:
    external_id = _get_latest_external_id(client, data_set_type)
...
def get_dataset_external_id(client: PowerOpsClient, data_set_type: DataSetType = "READ") -> str | None:
    external_id = _get_latest_external_id(client, data_set_type)
    dataset = client.cdf.data_sets.retrieve(external_id=external_id)
    if dataset is None:
        raise ValueError(f"Dataset with external_id '{external_id}' not found.")
